...
const MarketingAnimatedCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      ripple.className = 'marketing-ripple';
  ...
  return (
    <article ref={cardRef} className={`marketing-card ${className}`}>
      {children}
    </article>
  );
};
...
const MarketingOfferingsSection: React.FC = () => {
  ...
  return (
    <section className="bg-[#1D2255] py-16 sm:py-20 marketing-offerings-section">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div ref={cardsGridRef} className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 items-stretch">
          {offerings.map((item) => (
            <MarketingAnimatedCard
              key={item.title}
              className={`rounded-2xl border border-[#AAB4EA] p-5 sm:p-6 flex flex-col ${item.className}`}
            >
